 ...
 */
public class Ex19ShallowDeepCopyApp {

    public static void main(String[] args) {
        int[][] grid = {{1, 2, 3},{4, 5, 6}};
        int[][] gridCopyRef = coryReference(grid);

        gridCopyRef[0][0] = 100;
        traverse(grid);

        int[][] gridShallowCopy = shallowCopy(grid);
        gridShallowCopy[0][1] = 200;
        traverse(grid);

        int[][] gridDeepCopy = deepCopy(grid);
        gridDeepCopy[1][1] = 300;
        traverse(grid);
     ...
     *              a shallow copy.
     */
    public static int[][] coryReference(int[][] arr) {
        return arr;
    }
